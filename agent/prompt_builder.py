...
def build_memory_guidance(memory_enabled: bool = True, profile_enabled: bool = True) -> str:
    ...
    return frame + (
        ...
        "Route by scope: project-scoped facts (build commands, machine-"
        "specific paths, project credentials, repo quirks) belong in "
        "target='project' — that store loads only in sessions inside that "
        "repo and keeps the global budget free for cross-project facts."
    )
