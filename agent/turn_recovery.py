...
def compute_error_backoff(
    ...
    # Z.AI/Zhipu subscription quota windows ([1308]/[1310] "使用上限 … HH:MM 重置"
    # 429s, fork feature): no short backoff can bridge a multi-hour window —
    # sleep until the stated reset instant (+skew) and retry. Self-correcting:
    # each retry re-parses the fresh error, so a still-exhausted window yields
    # a small residual wait instead of a full reset.
    from agent.retry_utils import is_zai_subscription_quota_error, parse_zai_subscription_reset_wait
    _zai_subscription_wait = (
        parse_zai_subscription_reset_wait(api_error)
        if is_zai_subscription_quota_error(error=api_error)
        else None
    )
    if _zai_subscription_wait is not None and _retry_after is None:
        wait_time = _zai_subscription_wait
        _backoff_policy = "zai_subscription_reset"
    _adaptive = (is_rate_limited or is_zai_coding_overload) and _backoff_policy is None
...
def route_classified_error(
    ...
    # Subscription quota windows ([1308]/[1310], fork): raise the ceiling so the
    # reset-wait backoff in compute_error_backoff is reachable.
    from agent.retry_utils import (
        is_zai_subscription_quota_error, parse_zai_subscription_reset_wait,
        zai_subscription_quota_retry_ceiling,
    )
    _zai_subscription_wait = (
        parse_zai_subscription_reset_wait(api_error)
        if is_zai_subscription_quota_error(error=api_error) else None
    )
