...
def _memory_parts(agent: Any) -> List[str]:
    ...
    if agent._memory_store:
        # Upstream loop (memory + USER.md, gated individually).
        ...
        if agent._memory_enabled or agent._user_profile_enabled:
            try:
                ...
                pass
            # Linked project memory (fork, v2): read-only blocks from local
            # dependency repos that have their own project memory.
            try:
                linked_block = agent._memory_store.format_for_system_prompt(
                    "linked_projects"
                )
                if linked_block:
                    parts.append(linked_block)
            except Exception:
                pass
